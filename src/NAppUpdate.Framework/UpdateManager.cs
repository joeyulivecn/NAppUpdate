...
namespace NAppUpdate.Framework
{
    ...
    public sealed class UpdateManager
    {
        ...
        private UpdateManager()
        {
            ...
            string backupPath = Path.Combine(Path.GetDirectoryName(Config.DestinationFolder) ?? string.Empty, "Backup" + DateTime.Now.Ticks);
        ...
        public void CheckForUpdates()
        {
            ...
            using (WorkScope.New(isWorking => IsWorking = isWorking))
            {
                ...
                lock (UpdatesToApply)
                {
                    ...
                    {
                        //t.ProgressDelegate += status =>
                        //{
                        //    status.Message = (t as FileUpdateTask).LocalPath;
        ...
        public void ApplyUpdates(bool relaunchApplication, bool updaterDoLogging, bool updaterShowConsole)
        {
            ...
            lock (UpdatesToApply)
            {
                using (WorkScope.New(isWorking => IsWorking = isWorking))
                {
                    ...
                    // TODO: no need to cold update 
                    hasColdUpdates = false;
